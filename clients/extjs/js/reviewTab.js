...
async function loadTree(node, cb) {
  try {
    ...
    if (match) {
      ...
      packageGrant = curUser.packageGrants.find( g => g.packageId === packageId )
      if (packageGrant && packageGrant.accessLevel >= 3) {
        content.unshift({
          id: `${packageId}-assets-create-leaf`,
          action: 'asset-create',
          text: 'Add Asset...',
          cls: 'sm-tree-node-create',
          packageId: packageId,
          iconCls: 'sm-add-icon',
          qtip: 'Add an Asset to this Package',
          leaf: true
        })
      }
      cb(content, { status: true })
    ...
    if (match) {
      ...
      let content = r.stigs.map(stig => ({
        ...
        assetId: r.assetId,
        packageId: packageId,
        packageName: tree.getNodeById(`${packageId}-package-node`).attributes.packageName,
      ...
      if (packageGrant && packageGrant.accessLevel >= 3) {
        content.unshift({
          id: `${packageId}-assets-asset-create-leaf`,
          action: 'asset-stig-create',
          text: 'Attach STIG...',
          cls: 'sm-tree-node-create',
          assetId: assetId,
          packageId: packageId,
          iconCls: 'sm-add-icon',
          qtip: 'Attach a STIG to this Asset',
          leaf: true
        })
      }
      cb(content, { status: true })
